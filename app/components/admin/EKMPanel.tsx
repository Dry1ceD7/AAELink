  ...
  return (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--mm-main-bg)', color: 'var(--mm-text)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: 24, color: 'var(--mm-muted)' }}><Loader2 size={20} className="spin" /> Loading encryption data…</div>
        ) : (
          <>
            {tab === 'keys' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {keys.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: 24, opacity: 0.5, fontSize: 13 }}>No encryption keys configured. Add a key to enable BYOK encryption.</div>
                ) : keys.map(key => {
                  ...
                  return (
                    <div key={key.id} style={{ borderRadius: 12, border: '1px solid var(--mm-border)', overflow: 'hidden' }}>
                      <div onClick={() => setExpandedKey(expanded ? null : key.id)} style={{
            ...
            {tab === 'audit' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {events.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: 24, opacity: 0.5, fontSize: 13 }}>No key audit events recorded yet.</div>
                ) : events.map(ev => (
            ...
            {tab === 'settings' && (
              <div style={{ maxWidth: 520, display: 'flex', flexDirection: 'column', gap: 20 }}>
                {[
                  { label: 'Auto-rotate keys', desc: 'Automatically rotate encryption keys on a schedule', defaultOn: true },
                  { label: 'Rotation interval', desc: 'Days between automatic key rotations', defaultOn: false, isSelect: true },
                  { label: 'Require approval for revocation', desc: 'Key revocation requires a second admin approval', defaultOn: true },
                  { label: 'Log all decrypt operations', desc: 'Record every decryption operation in the audit log', defaultOn: false },
                ].map(s => (
                  <div key={s.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 14, borderRadius: 10, border: '1px solid var(--mm-border)' }}>
                    ...
                    {s.isSelect ? (
                      ...
                      <div style={{ width: 44, height: 24, borderRadius: 12, background: s.defaultOn ? '#2bac76' : 'var(--mm-hover-bg)', cursor: 'pointer', position: 'relative' }}>
                        <div style={{ width: 18, height: 18, borderRadius: '50%', background: '#fff', position: 'absolute', top: 3, left: s.defaultOn ? 23 : 3, transition: 'left 200ms ease' }} />
                      </div>
                    )}
                  </div>
                ))}
      ...
      {/* Add Key Modal */}
      {showAddKey && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 3000, display: 'grid', placeItems: 'center', animation: 'slack-modal-in 200ms var(--slack-ease-bounce) forwards' }} onClick={() => setShowAddKey(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--mm-main-bg)', borderRadius: 16, padding: 24, width: 440, boxShadow: 'var(--slack-shadow-modal)' }}>
            <h3 style={{ margin: '0 0 16px', fontSize: 16, fontWeight: 800 }}>Add Encryption Key</h3>
            <p style={{ fontSize: 13, opacity: 0.6, marginBottom: 16 }}>Select a cloud KMS provider to connect your customer-managed key.</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {PROVIDERS.map(p => (
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
              <button onClick={() => setShowAddKey(false)} style={{ background: 'var(--mm-hover-bg)', border: 'none', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', color: 'var(--mm-text)', fontSize: 13 }}>Cancel</button>
              <button onClick={() => { setShowAddKey(false); setSelectedProvider('') }} style={{ background: '#8b5cf6', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer', opacity: selectedProvider ? 1 : 0.5 }}>Continue Setup</button>
            </div>
          </div>
        </div>
      )}
