            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ textAlign: 'left', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
                  ...
                  <th style={{ padding: '6px' }}>Triggered By</th>
              ...
              <tbody>
                {executions.slice(0, 20).map(ex => (
                  <tr key={ex.id} style={{ borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
                    <td style={{ padding: '6px' }}>{functions.find(f => f.id === ex.function_id)?.name || ex.function_id.slice(0, 8)}</td>
                    <td style={{ padding: '6px' }}>{statusBadge(ex.status)}</td>
                    <td style={{ padding: '6px', fontSize: 11 }}>{ex.triggered_by || '—'}</td>
                    <td style={{ padding: '6px', fontSize: 11 }}>
                      {ex.completed_at && ex.created_at ? `${Math.round((ex.completed_at - ex.created_at) / 1000)}s` : '—'}
                    </td>
                    <td style={{ padding: '6px', fontSize: 11 }}>{ex.created_at ? new Date(ex.created_at).toLocaleString() : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
