...
export default function WorkflowBuilder({ onClose }: { onClose: () => void }) {
  ...
  const card: React.CSSProperties = {
    ...
    transition: 'all 200ms var(--slack-ease-out)',
  }
              ...
              {workflows.map(wf => {
                ...
                return (
                  <div key={wf.id} style={card}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = '#4361EE'; e.currentTarget.style.boxShadow = '0 2px 12px rgba(67,97,238,0.1)' }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--mm-border)'; e.currentTarget.style.boxShadow = 'none' }}
                    onClick={() => openEdit(wf)}
                  >
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--mm-main-bg)', color: 'var(--mm-text)' }}>
      <div style={{ padding: '12px 20px', borderBottom: '1px solid var(--mm-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '32px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{ width: '100%', maxWidth: 480, marginBottom: 8 }}>
          <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, opacity: 0.4, marginBottom: 8 }}>
            Trigger
          </div>
          {selectedTrigger ? (
        ...
        {editingWf.steps.length > 0 && <div style={{ width: 2, height: 24, background: 'var(--mm-border)' }} />}
            ...
            <div key={step.id} style={{ width: '100%', maxWidth: 480 }}>
              ...
              {idx < editingWf.steps.length - 1 && (
                <div style={{ display: 'flex', justifyContent: 'center' }}>
                  <div style={{ width: 2, height: 20, background: 'var(--mm-border)' }} />
                </div>
              )}
            </div>
          )
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginTop: 8 }}>
          {editingWf.steps.length > 0 && <div style={{ width: 2, height: 20, background: 'var(--mm-border)' }} />}
        </div>
