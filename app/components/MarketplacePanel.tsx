...
export const MarketplacePanel = memo(function MarketplacePanel({ workspaceId }: { workspaceId: string }) {
  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      ...
      {tab === 'publish' ? (
        /* ── Publish form ────────────────────────────────────── */
        <form onSubmit={handlePublish} style={{ maxWidth: 560 }}>
          <p style={{ margin: '0 0 16px', fontSize: 14, color: 'var(--mm-muted)' }}>
          ...
          <div className="mm-settings-form-row">
            ...
            <textarea id="pub-desc" className="mm-settings-input" value={pubDesc} rows={3}
              onChange={e => setPubDesc(e.target.value)} maxLength={500} placeholder="What does your plugin do?"
              style={{ resize: 'vertical', minHeight: 60 }} />
          </div>
          <div style={{ display: 'flex', gap: 12 }}>
            <div className="mm-settings-form-row" style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <div className="mm-settings-form-row" style={{ width: 80 }}>
              <label className="mm-settings-form-label" htmlFor="pub-emoji">Icon</label>
              <input id="pub-emoji" className="mm-settings-input" value={pubEmoji}
                onChange={e => setPubEmoji(e.target.value)} maxLength={4} style={{ textAlign: 'center', fontSize: 18 }} />
            </div>
            <div className="mm-settings-form-row" style={{ width: 120 }}>
              <label className="mm-settings-form-label" htmlFor="pub-bg">Color</label>
              <input id="pub-bg" type="color" value={pubBg}
                onChange={e => setPubBg(e.target.value)} style={{ width: '100%', height: 34, border: 'none', padding: 0, cursor: 'pointer' }} />
            </div>
          </div>
          {pubErr && <div className="mm-auth-alert mm-auth-alert--error" role="alert" style={{ marginTop: 8 }}><span>{pubErr}</span></div>}
          {pubOk && <div className="mm-auth-alert mm-auth-alert--success" role="status" style={{ marginTop: 8 }}><CheckCircle size={14} /><span>{pubOk}</span></div>}
                      ...
                      {isInstalled ? (
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button type="button" className="marketplace-install-btn installed" disabled>
                            <CheckCircle size={12} style={{ marginRight: 4 }} /> Installed
                          </button>
                          <button type="button" className="marketplace-install-btn"
                            style={{ borderColor: '#d24b4e', color: '#d24b4e' }}
