...
import DocumentAssemblyPanel from './DocumentAssemblyPanel'
import {
  FileText, Layers, Building2, Sparkles, Upload, Download, Trash2, Eye,
  Search, ScanText, X
} from 'lucide-react'
...
type DocTab = 'files' | 'templates' | 'clients' | 'assembly'
...
export function DocumentsPanel({ workspaceId }: { workspaceId: string }) {
  ...
  const tabs: { key: DocTab; label: string; icon: React.ReactNode }[] = [
    ...
    { key: 'templates', label: 'Templates', icon: <Layers size={15} /> },
    { key: 'clients', label: 'Clients', icon: <Building2 size={15} /> },
    { key: 'assembly', label: 'Assembly', icon: <Sparkles size={15} /> },
  ]
  ...
  return (
    <div className="docs-tabbed-panel">
      ...
      <div className="docs-tab-content">
          ...
          <div className="docs-files-tab">
            ...
            >
              ...
              <span className="docs-drop-hint">
                Supports PDF, Word, Text, CSV, Markdown, and images. Uploads persist automatically.
              </span>
            </div>
        ...
        {/* ── TEMPLATES TAB ── */}
        {tab === 'templates' && <TemplatesPanel workspaceId={workspaceId} />}
        ...
        {tab === 'assembly' && (
          <DocumentAssemblyPanel
            ...
            onDocumentCreated={() => { void loadDocs(); setTab('files') }}
          />
        )}
      </div>
