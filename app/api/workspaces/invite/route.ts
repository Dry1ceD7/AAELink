...
export async function POST(req: NextRequest) {
  ...
  const { rows: wsRows } = await pool.query(`SELECT id, display_name FROM aaelink.workspaces WHERE id = $1`, [wsId])
  ...
  return NextResponse.json({
    ...
    workspace_name: (wsRows[0] as { display_name: string }).display_name
  })
}
