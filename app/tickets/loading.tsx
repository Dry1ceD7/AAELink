/** Legacy /tickets URL: brief load before client redirect to home tickets. */
export default function TicketsLoading() {
  return (
    <main
      className="mm-app-loading"
      role="status"
      aria-live="polite"
      aria-busy="true"
      aria-labelledby="tickets-route-loading-label"
    >
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
        <div className="mm-route-loading" aria-hidden="true">
          <span className="mm-route-loading-bar" />
          <span className="mm-route-loading-bar mm-route-loading-bar--mid" />
          <span className="mm-route-loading-bar mm-route-loading-bar--short" />
        </div>
        <p id="tickets-route-loading-label" style={{ margin: 0, fontWeight: 600, color: 'var(--aae-navy-deep)' }}>
          Opening tickets
        </p>
      </div>
    </main>
  )
}
