            ...
            <tbody>
              {mappings.slice(0, 50).map((m, i) => (
                <tr key={i}>
                  ...
                  <td style={{ fontFamily: 'monospace', fontSize: 11 }}>{m.source_user_id}</td>
                  <td style={{ fontFamily: 'monospace', fontSize: 11 }}>{m.target_user_id}</td>
                  <td style={{ fontSize: 11 }}>{m.mapped_at ? new Date(m.mapped_at).toLocaleString() : '—'}</td>
                </tr>
              ))}
            </tbody>
          </DataTable>
