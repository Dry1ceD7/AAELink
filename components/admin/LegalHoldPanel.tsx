...
export default function LegalHoldPanel({ onClose }: { onClose: () => void }) {
  ...
  const [settings, setSettings] = useState<Record<string, boolean>>({
    ...
    'Auto-notify custodians': true,
    'Include deleted messages': false,
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
        {tab === 'holds' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {loading ? (
              ...
              <EmptyState
                icon={<Scale size={40} aria-hidden="true" />}
                title="No legal holds yet"
                description="Create a hold to preserve data for legal proceedings — messages, files, and metadata from selected custodians and channels are kept regardless of retention policies."
              />
            ) : holds.map(hold => {
              ...
              return (
                <div key={hold.id} style={{ borderRadius: 12, border: '1px solid var(--mm-border)', overflow: 'hidden' }}>
                  <div onClick={() => setExpandedHold(expanded ? null : hold.id)}
        ...
        {tab === 'exports' && (
          <EmptyState
            ...
            title="Export history will appear here"
            description='Use the "Export Data" button on any legal hold to generate a compliance export.'
          />
        )}
        ...
        {tab === 'settings' && (
          <div style={{ maxWidth: 520, display: 'flex', flexDirection: 'column', gap: 16 }}>
            {[
              ...
              const labelId = `legalhold-setting-${s.label.replace(/\s+/g, '-').toLowerCase()}`
              return (
        ...
        footer={
          <>
            ...
            <button
              ...
              style={{ background: '#e01e5a', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
            >
              Create Hold
            </button>
          </>
