 ...
 *
 * When a `bus` is supplied (WS-enabled home shell), presence updates ride the
 * gateway via the bus and the SSE connect is skipped. The DND fetch loop
 * stays — DND is set explicitly by the user, not part of the heartbeat.
 */
export function usePresenceListener(workspaceId: string, bus?: RealtimeEventBus) {
  ...
  // ── WS-bus path: skip SSE entirely when bus is supplied ─────────────
  useEffect(() => {
    if (!workspaceId || !bus) return
    const off = bus.subscribePresence(change => {
      setPresenceMap(prev => {
        const existing = prev[change.userId] ?? 0
        if (change.lastSeen <= existing) return prev
        return { ...prev, [change.userId]: change.lastSeen }
      })
    })
    return () => {
      off()
  ...
  // ── Legacy SSE fallback (only when no bus is supplied) ──────────────
  useEffect(() => {
    if (!workspaceId || bus) return
