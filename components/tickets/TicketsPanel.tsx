...
export function TicketsPanel({
  ...
  // ── Guarded ticket transition state ─────────────────────────────────────
  ...
  /**
   * Drive a ticket through the guarded state machine at /api/tickets/[id]/transition.
   * On HTTP 409 the server returns a structured guard error (resolution_note_required,
   * resolution_category_required, reopen_reason_required, four_eyes_violation, …) —
   * we surface those to the user via TransitionDialog (a focus-trapped modal) and
   * re-submit with the collected metadata. Returns true on success, false on rejection.
   */
  async function transitionTicket(
    ...
    if (!viewerIsIt) return false
    setActionError('')
    const current = tickets.find(t => t.id === id)
    const from = (current?.status || 'open') as TicketStatus

    const res = await apiFetch(`/api/tickets/${encodeURIComponent(id)}/transition`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (res.status === 409) {
      ...
      // Pick the right modal mode based on the guard code
      ...
      if (!mode) {
        // Non-recoverable guard failure — surface the hint and stop
        setActionError(hint || `Cannot move to ${to}.`)
        return false
      }
      ...
      // Recurse with collected metadata
      return transitionTicket(id, to, { ...extras, ...collected })
    }

    if (!res.ok) {
      setActionError(await messageFromFailedResponse(res, 'Could not transition the ticket.'))
      return false
    }

    // Refresh the ticket from the source of truth
    const detailRes = await apiFetch(`/api/tickets/${encodeURIComponent(id)}`)
    if (detailRes.ok) {
      const detail = (await detailRes.json()) as { ticket: Ticket }
      setTickets(current =>
        current.map(t => (t.id === id ? { ...t, ...detail.ticket, status: detail.ticket.status, createdBy: detail.ticket.createdBy ?? t.createdBy } : t))
      )
    } else {
      // Fallback: optimistic patch
      setTickets(current => current.map(t => (t.id === id ? { ...t, status: to } : t)))
    }
    if (selectedId === id) void loadDetail(id, { soft: true })
    return true
  }
        ...
        <>
          ...
          {viewMode === 'kanban' && (
            <TicketKanbanBoard
              ...
              onStatusChange={(id, status) => transitionTicket(id, status as Ticket['status'])}
              userMap={userMap}
            />
          )}
          ...
          {/* ── Transition Dialog (state-machine guard prompts) ── */}
          {transitionDialog && (
            <TransitionDialog
              open={true}
                      ...
                      <div className="ticket-controls" style={{ marginTop: 10 }}>
                        <label>
                          ...
                          <select
                            ...
                            disabled={detailBusy}
                            onChange={e => void transitionTicket(selected.id, e.target.value as Ticket['status'])}
                          >
                            <option value="open">Open</option>
                            <option value="pending">Pending</option>
                            <option value="in_progress">In progress</option>
