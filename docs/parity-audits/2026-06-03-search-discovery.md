...
| 8 | `before:<date>` modifier | yes | yes | both routes | `searchFilters.test.ts` | ✅ Full | `YYYY-MM-DD` validated; `messages` uses `<=` end-of-day, `advanced` uses `<` |
| 9 | `after:<date>` modifier | yes | yes | both routes | `searchFilters.test.ts` | ✅ Full | `>=` start-of-day in `messages` route |
